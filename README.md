...
[x]  RDBMS is a Relational Database Management System, its the Database software itself that manages storage: querying and updating. SQL is Structured Query Language -  It's a platform to use the RDBMS - a family of closely related languages which typically is used to query the RDBMS.

1. Why do tables need a `primary key`?
[x] It defines a Foreign Key relationship between a child table and a parent table, so that multiple tables can be queried together

1. What is the name given to a table column that references the primary key on another table.
[x] Foreign Key

1. What do we need in order to have a _many to many_ relationship between two tables.
A 3rd table containing all the columns from both - when both tables can have many of eachother.
...
- [x] Feel free to name the tables and fields anything you want. **Add relationships** as you see fit.
